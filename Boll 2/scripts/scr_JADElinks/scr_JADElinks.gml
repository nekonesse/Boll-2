function basicLink(_name, _var, _can_input=true, _maxoutputs=NaN) constructor {
	name = _name;
	myvar = _var;
...
function bindingLink(_name, _var, _maxoutputs=NaN) constructor {
	name = _name;
	myvar = _var;
	can_input = false;
	outputs = [];
	...
	static draw = function(_x,_y) {
		...
		var i=0;
		while (i < array_length(outputs)) {
			if !struct_exists(oJADEController.object_uuids,outputs[i]) {
				array_delete(outputs,i,1);
			} else {
				if (mbleft) && (point_in_rectangle(curs_x, curs_y, _x+8+188-16,_y+24+6+offset,_x+8+188+4,_y+24+6+12+offset)) {
					array_delete(outputs,i,1);
		...
		repeat(array_length(outputs)) {
			draw_gui(_x+8,_y+24+offset,188,24,color,1,true);
			ScribblejrShrinkExt(outputs[i],fa_left,fa_top,global.rulerGold,1,172,20).Draw(_x+16, _y+24+6+offset)
			draw_sprite(spr_JADEdeleteiconsmall,0,_x+8+188-16,_y+24+6+offset);
