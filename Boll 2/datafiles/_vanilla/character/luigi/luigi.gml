#define datalist
spriteEvents=split_string("idle,walk,run,wait,lookUp,crouchIdle,crouchWalk,crouchJump,crouchFall,crouchFireToss,crouchBonk,crouchFireToss,victory,hurt,dead,brake,jump,fall,bonk,runJump,runJumpFall,wallSlide,wallJump,groundPound,groundPoundFall,slopeSlide,carryIdle,carryWalk,carryRun,carryLookUp,carryJump,carryFall,carryBonk,carrySpinJump,carrySpinJumpFall,carryCrouchIdle,carryCrouchWalk,carryCrouchJump,carryCrouchFall,carryCrouchBonk,carryKick,carryAirKick,roll,swim,swimPaddle,carrySwim,carryPaddle,spinJump,spinJumpFall,pushing,balancing,fireToss,electrocute,gateClimbing,flagPole,hang,monkeyBars,boarding,snowboarding,downPipeEnter,downPipeExit,upPipeEnter,upPipeExit,sidePipeEnter,sidePipeExit,doorEnter,doorExit",",");
miscSprites=split_string("shield",",");
sound_list=split_string("damage,shielddamage,die,galspin,fireball,flip,jump,kick,pound,select,skid,spin,spinbounce,spinjump,stomp,swim,wallkick",",");
...
if (state == "pound") || (alarm_get(2)) || (hurt) || (stun) || (finish && posed && no_move_prev) {
	no_move = true;
}

can_grab = true;
if (state == "pound") || (spinjump) || (hurt) || (stun) || (finish && posed && no_move_prev) {
	can_grab = false;
}
...
if ((state == "" || state=="crouch") && !hurt && !stun && apress && canjump > 0 && !spinjump) && !piped && !(underwater) {
	...
	starmanjump = false;
	state = "jump"
	vsp = -(5.4+(clamp(abs(hsp)/3.14,0.5,1.7) * 1.2)+(bool(poundjump)+0.5)); //preform the actual jump
...
#region Spinjumping & Galaxy Spin
...
if (cpress && !is_grabbing) && !(stun) {
	if (grounded) {
	...
	} else if !(wallkick) && (state != "pound") {
		playsfx(charmName+"galspin");
		galspin = true;
		spinjump = false;
		state = "jump";
		grounded = false;
		...
		make_particle(pSmoke, x, y + hit_sizey, depth + 5, 1, 0, (y-yprevious)/1.5, 0, 0.2);
	}
}
...
if (coll) && !(coll.no_dam) && (coll.phaseid!=id) {
	if (coll) && ((!slopesliding && state != "pound" && !(galspin && vsp < 0)) || coll.damage_on_contact) && !(invincible_type && invincible_timer) {
		if (coll.deal_dam) {
			if !(shielded) {
				stopsfx(charmName+"skid")
				stopsfx(charmName+"damage")
	...
	} else if (slopesliding) || (galspin && vsp < 0) {
