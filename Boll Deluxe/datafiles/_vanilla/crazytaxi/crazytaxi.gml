...
if (size == "basic") {
	...
	if !(grounded) {
		if !(ramping) sprite_angle=median(45,-45,sprite_angle+(-vsp/4))
	} else {
		sprite_angle=lerp(sprite_angle,colangle*xsc,0.33)
	}
	
	if esign(move,xsc)!=xsc && !braking {
		braking=30
		brakedir=xsc
		xsc=-xsc
	}
	...
	var enemy=collision_rectangle(x-hit_sizex-1,y,x+hit_sizex+1,y+hit_sizey,oEnemy,false,true)
	if (enemy) && (vsp >= 0) {
		if (enemy) {
		    signal_emit(enemy.enemyStomped, id, charmName);
		}
	}
	
	player_slide_sonic(0.125, 1, 0.078125, 0.3125);
}
